...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // ACTION 4: Play Page Generator (Handles both Movies & TV Series)
        if (action === 'play') {
            const sid = req.query.id;
            let detailPath = req.query.detail || '';
            let se = req.query.se || '0';
            let ep = req.query.ep || '0';
            ...
            // Try fetching with provided se/ep (e.g. 0,0 for movies)
            try {
            ...
            // Fallback to se=1, ep=1 if episode 0 had no streams (for TV Series)
            ...
            if (!streams.length) {
                ...
                <!DOCTYPE html><html><head><title>No Stream Available</title><style>body{background:#0f172a;color:#fff;font-family:sans-serif;padding:30px;text-align:center;}a{color:#38bdf8;font-size:18px;}</style></head><body>
                    <h2>⚠️ Stream links currently unavailable for this title on source CDN.</h2>
